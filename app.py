...
def train_models(df):
    # ── Gunakan sample 5000 baris agar hemat RAM ──────────────
    df_sample = df.sample(min(5000, len(df)), random_state=SEED)
    ...
    # ── CLUSTERING (pakai sample) ─────────────────────────────
    X_cl   = df_sample[amen_cols + struct_cols].fillna(0)
    scaler = StandardScaler()
    X_sc   = scaler.fit_transform(X_cl)

    # K optimal: cek range 2–6 saja (lebih cepat)
    sil_scores = []
    for k in range(2, 7):
    ...
    # Fit KMeans final pada sample, predict ke seluruh data
    km_final = KMeans(n_clusters=best_k, random_state=SEED, n_init=10)
    km_final.fit(X_sc)

    df = df.copy()
    X_all = scaler.transform(df[amen_cols + struct_cols].fillna(0))
    df['cluster'] = km_final.predict(X_all)
    ...
    # Tambahkan pca ke df utama (isi 0 untuk baris di luar sample)
    df['pca1'] = 0.0
    df['pca2'] = 0.0
    df.loc[df_sample.index, 'pca1'] = df_sample['pca1'].values
    df.loc[df_sample.index, 'pca2'] = df_sample['pca2'].values
    ...
    # Fitur inti saja (tanpa semua amen_cols) agar hemat RAM
    feat_cols = [
        ...
        # Hanya 15 amenitas paling penting
