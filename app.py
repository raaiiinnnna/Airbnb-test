...
def train_models(df):
    # Salin df dan siapkan semua kolom dulu, baru ambil sample
    df = df.copy()
    ...
    # ── STEP 1: CLUSTERING ───────────────────────────────────
    # Fit pada sample 5000, predict ke seluruh data
    samp_idx  = df.sample(min(5000, len(df)), random_state=SEED).index
    X_cl_all  = df[amen_cols + struct_cols].fillna(0)
    scaler    = StandardScaler()
    X_sc_all  = scaler.fit_transform(X_cl_all)

    # Posisi numerik index sample
    all_idx_list = df.index.tolist()
    samp_pos  = [all_idx_list.index(i) for i in samp_idx]
    X_sc_samp = X_sc_all[samp_pos]

    # Cari K optimal
    sil_scores = []
    for k in range(2, 7):
        km  = KMeans(n_clusters=k, random_state=SEED, n_init=5)
        lbl = km.fit_predict(X_sc_samp)
        sil_scores.append(silhouette_score(X_sc_samp, lbl,
    ...
    km_final.fit(X_sc_samp)
    df['cluster'] = km_final.predict(X_sc_all)

    rank_labels = {0:'🟢 Ekonomis',1:'🔵 Standar',2:'🟠 Nyaman',
                   3:'🔴 Premium',4:'⭐ Mewah'}
    ...
    X_pca    = pca.fit_transform(X_sc_samp)
    explained = pca.explained_variance_ratio_ * 100
    df['pca1'] = 0.0
    df['pca2'] = 0.0
    df.loc[samp_idx, 'pca1'] = X_pca[:, 0]
    df.loc[samp_idx, 'pca2'] = X_pca[:, 1]

    # ── STEP 2: Encode — dilakukan di df utama ────────────────
    ...
    # ── STEP 3: Ambil sample SETELAH semua kolom siap ─────────
    df_sample = df.loc[samp_idx].copy()

    # Fitur inti saja agar hemat RAM
    feat_cols = [
        ...
        'amen_pool','amen_hot_tub','amen_gym','amen_washer',
        'amen_dryer','amen_free_parking_on_premises',
        'amen_dedicated_workspace','amen_self_check_in',
        'amen_pets_allowed','amen_air_conditioning',
        'amen_patio_or_balcony','amen_bathtub','amen_elevator',
        'amen_bbq_grill','amen_long_term_stays_allowed',
    ]
    feat_cols = [c for c in feat_cols if c in df_sample.columns]

    ml_df = df_sample[feat_cols + ['price']].dropna(subset=['price']).copy()
    ml_df[feat_cols] = ml_df[feat_cols].fillna(ml_df[feat_cols].median())

    X, y = ml_df[feat_cols], ml_df['price']
    Xtr, Xte, ytr, yte = train_test_split(X, y, test_size=0.2, random_state=SEED)

    # Random Forest ringan
    rf = RandomForestRegressor(
        n_estimators=80, max_depth=10,
        min_samples_leaf=5, max_features='sqrt',
        random_state=SEED, n_jobs=1        # n_jobs=1 hemat RAM
    )
    rf.fit(Xtr, ytr)
    yp_rf = rf.predict(Xte)

    # Gradient Boosting ringan
...
def tab_peta(df):
    ...
    # Batasi 800 titik agar tidak berat di browser
    smp = df[df['price'] < 600].sample(min(800, len(df)), random_state=SEED).copy()
    ...
    def price_color(p):
        ...
        elif p < 150: return '#3498db'
        elif p < 300: return '#e67e22'
        else:         return '#e74c3c'
    ...
    st_folium(m, width=1050, height=520, returned_objects=[])

    # Legenda
    st.markdown("**Legenda:**")
    if color_by == 'cluster_label':
        items = [(lbl, col) for lbl, col in cluster_colors.items()
                 if lbl in df['cluster_label'].unique()]
    elif color_by == 'price':
    ...
    st.caption(f"Menampilkan {len(smp):,} titik dari {len(df):,} listings · "
               f"Klik titik untuk detail · Hover untuk harga & cluster")
